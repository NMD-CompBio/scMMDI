...
class AttVEncoder(BaseEncoder):
    def __init__(
        ...
        self.mask_embed = nn.Sequential(
            nn.Linear(3, 64),
            nn.ReLU(),
            nn.Linear(64, mask_embed_dim),
            nn.LayerNorm(mask_embed_dim),
            nn.Tanh()
        )
        #self.mask_weights = nn.ParameterDict({
            #k: nn.Parameter(torch.tensor(0.3, requires_grad=True)) for k in incs
        #})
        self.mask_weight = nn.Parameter(torch.tensor(1.0).sigmoid())

    def forward(self, batch: GMINIBATCH) -> FRES:

        # 处理每个模态的掩码并生成 mask_embed
        mask_embeds = {}
        for k, mask_part in batch['mask'].items():

            # 生成 one-hot 编码的掩码输入 (假设 mask_part 是一个掩码矩阵)
            mask_embed_input = F.one_hot((mask_part + 1).long(), num_classes=3).float()


            # 将 one-hot 编码输入传入 mask_embed 层
            mask_embed = self.mask_embed(mask_embed_input)


            # L2 范数正则化
            mask_embed = mask_embed / (torch.norm(mask_embed, dim=-1, keepdim=True) + 1e-6)
            mask_embeds[k] = mask_embed * self.mask_weight
            # 标准化
            #mask_embed = (mask_embed - mask_embed.mean(dim=0, keepdim=True)) / (
                    #mask_embed.std(dim=0, keepdim=True) + 1e-6
            #)
        ...
        for k, mlpi in self.unshared_mlps.items():
            # 获取每个模态的有效掩码
            mask_part = batch['mask'][k]
            valid_mask = (mask_part == 0.0).float().unsqueeze(-1).expand_as(batch["input"][k])
            # 处理当前模态数据并拼接 mask_embed
            data = batch["input"][k] * valid_mask
            combined_input = torch.cat([data, mask_embeds[k]], dim=-1)

            embeds_dedicated[k] = mlpi(
                combined_input, [], [batch["blabel"]] if self.nbatch else []
            )
        h, att = self.fusion(embeds_dedicated, batch["mask"])

        _, z = self.mapper(h)

        #embeds_dedicated_unobs = {}
        #for k, mlpi in self.unshared_mlps.items():
            # 获取每个模态的有效掩码
            #mask_part_unobs = batch['mask'][k]
            #valid_mask_unobs = (mask_part == 0).float().unsqueeze(-1).expand_as(batch["input"][k])
            # 处理当前模态数据并拼接 mask_embed
            #data_unobs = batch["input"][k] * valid_mask_unobs
            #combined_input_unobs = torch.cat([data_unobs, mask_embeds[k]], dim=-1)

            #embeds_dedicated_unobs[k] = mlpi(combined_input_unobs, [], [batch["blabel"]] if self.nbatch else [])

        #h_unobs, att_unobs = self.fusion(embeds_dedicated_unobs,  batch["mask"])
        #_, z_unobs = self.mapper(h_unobs)

        return {"z": z, "att": att, "mask_embed": mask_embed}

    def step(self, batch: GMINIBATCH) -> Tuple[FRES, LOSS]:
        z = self.forward(batch)
        loss_kl = calc_kl_normal(z["z"])
        loss_kl = self.reductor(loss_kl, dim=1).mean()
        reg_loss = 0.01 * sum(w.pow(2).sum() for w in self.mask_weights.values())
        return z, {"kl": loss_kl + reg_loss}
...
class GraphEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        self.incs = incs
        self.mask_embed = nn.Parameter(torch.zeros(vnum, mask_embed_dim))
        self.conv = GraphConv()
        self.loc = nn.Linear(out_features + mask_embed_dim, out_features)
        self.std_lin = nn.Linear(out_features + mask_embed_dim, out_features)

        if not zero_init:
            nn.init.trunc_normal_(self.vrepr, std=0.1)

    def forward(self, batch: GMINIBATCH, training: bool = True) -> Dict[str, Normal]:
        sidx, tidx, esgn, _, enorm = batch["normed_subgraph"]
        ptr = self.conv(self.vrepr, sidx, tidx, enorm, esgn)  # [vnum, out_features]

        # 获取模态 mask 信息（以 batch 为主）
        mask = batch["mask"]  # dict[str, Tensor], each: [batch_size]
        batch_size = next(iter(mask.values())).shape[0]
        device = ptr.device

        # 构造一个每个模态的 mask embedding 向量，加权平均
        mask_weights = []
        for k in self.incs:
            m = mask[k]  # shape: [batch]
            weight_k = (m == 0.0).float().mean()  # scalar
            weight_k = weight_k.view(1, 1)  # [1, 1]
            mask_weights.append(weight_k)

        mask_weights = torch.cat(mask_weights, dim=1)  # [1, num_modalities]
        # 取出模态嵌入（我们先假设3个模态在前3行）
        mask_embed_modal = self.mask_embed[:len(self.incs)]  # [3, mask_embed_dim]
        mask_embed = torch.sum(mask_embed_modal * mask_weights.transpose(0, 1), dim=0, keepdim=True)  # [1, dim]

        # 将 mask_embed broadcast 成 [vnum, dim]
        mask_embed = mask_embed.expand(ptr.shape[0], -1)  # [vnum, mask_embed_dim]

        # 拼接图卷积输出和 mask embedding
        combined_input = torch.cat([ptr, mask_embed], dim=-1)  # [vnum, out_features + mask_embed_dim]

        loc = self.loc(combined_input)
        std = F.softplus(self.std_lin(combined_input)) + 1e-7
        vdist = Normal(loc, std)
        vsample = vdist.rsample()

        return {"v": vdist, "vsample": vsample}

    def step(self, batch: GMINIBATCH) -> Tuple[Dict[str, Normal], LOSS]:
        fres = self.forward(batch, training=True)
