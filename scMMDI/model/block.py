...
class GatedAttentionFusion(nn.Module):
    ...
    def forward(
        ...
        mask_dict: Optional[OrderedDict[str, T]] = None,
    ) -> Tuple[T, T]:

        h = torch.stack([h_dict[n] for n in self.incs], dim=1)
        batch_size, n_omics, embed_dim = h.shape
        if self.omic_embed:
            h = h + self.omic_vec
        att = self.att1(h) * self.att2(h)

        if mask_dict is not None:
            #mask = torch.stack([mask_dict[n] for n in self.incs], dim=1).unsqueeze(-1)  # [B, N, 1]
            mask = torch.stack([mask_dict[n] for n in self.incs], dim=1)
            mask = mask.unsqueeze(-1)
            att[mask == -1.0] -= torch.inf
            #att[mask == 0.0] -= torch.inf# 缺失模态 attention 设为负无穷

            print(f"Mask shape: {mask.shape}, values: {mask.unique().tolist()}")
