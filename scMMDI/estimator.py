import warnings
from typing import Sequence, Optional, Union, Mapping, Tuple, Literal, Any, Dict
...
class scMMDI:
    def __init__(
        ...
        verbose: int = 2,
        deterministic: bool = True,  # True for reproducibility
        ...
        loss_weight_mnn: float = 0.5,  # 新增：MNN 损失权重
        shared_feature_keys: Optional[Mapping[str, Sequence[int]]] = None,  # 新增：共享特征的 obsm 键
        shared_nfeatures: Optional[int] = None,  # 新增：共享特征数量
        shared_indices: Optional[Mapping[str, Sequence[int]]] = None,  # 新增：共享特征索引
        n_knn: int = 30,  # 新增：MNN 的近邻数

        # 掩码相关参数
        p_random: float = 0.08,  # 新增：随机掩码概率
        p_modal: Optional[Sequence[float]] = None,  # 新增：模态缺失概率
    ):
        # TODO: add docstring!
        ...
        self.shared_feature_keys_ =  shared_feature_keys
        self.shared_nfeatures_ = shared_nfeatures
        self.shared_indices_ = shared_indices
        self.n_knn_ = n_knn

        # Mask parameters
        self.p_random_ = p_random
        self.p_modal_ = p_modal


    def fit(self, mdata: MuData, key_add: str = "scMMDI") -> None:
        ...
        # 计算共享特征维度
        self.shared_nfeatures_ = {}
        for k in mdata.mod.keys():
            if k == 'rna':
                if 'shared_rna_adt' in mdata.mod[k].obsm:
                    self.shared_nfeatures_['shared_rna_adt'] = mdata.mod[k].obsm['shared_rna_adt'].shape[1]
                if 'shared_rna_atac' in mdata.mod[k].obsm:
                    self.shared_nfeatures_['shared_rna_atac'] = mdata.mod[k].obsm['shared_rna_atac'].shape[1]
            elif k == 'protein' and 'shared_rna_adt' in mdata.mod[k].obsm:
                self.shared_nfeatures_['shared_rna_adt'] = mdata.mod[k].obsm['shared_rna_adt'].shape[1]
            elif k == 'atac' and 'shared_rna_atac' in mdata.mod[k].obsm:
                self.shared_nfeatures_['shared_rna_atac'] = mdata.mod[k].obsm['shared_rna_atac'].shape[1]

        if not self.shared_nfeatures_:
            raise ValueError("No shared features found in mdata.obsm (shared_rna_adt or shared_rna_atac).")

        shared_feature_dims = self.shared_nfeatures_
        ...
        loader_train = get_dataloader(
            ...
            repeat_sample=True,
            drop_last=True,
            shared_nfeatures=self.shared_nfeatures_,
            shared_indices=self.shared_indices_,
            shared_feature_keys=['shared_rna_adt', 'shared_rna_atac'],
        ...
        loader_valid = get_dataloader(
            ...
            shared_nfeatures=self.shared_nfeatures_,
            shared_indices=self.shared_indices_,
            shared_feature_keys=['shared_rna_atac','shared_rna_adt'],
        )
        ...
        if not hasattr(self, "model_"):  # can reuse trained model
            ...
            self.model_ = MMAAVINET(
                ...
                shared_feature_dims=shared_feature_dims,
                #p_feat=self.p_feat_,
                #p_modal=self.p_modal_,
                p_random=self.p_random_,
                p_modal=self.p_modal_,
            )
        ...
        loader_all_ = get_dataloader(
            ...
            shared_nfeatures=self.shared_nfeatures_,
            shared_indices=self.shared_indices_,
            shared_feature_keys=['shared_rna_atac'],
        ...
        self.model_.eval()
        with torch.no_grad():
            for batch in loader_all_:
                if "mask" in batch:
                    if isinstance(batch["mask"], dict):
                        # 按模态处理掩码，将每个张量移动到 device
                        masks = {
                            modality: batch["mask"][modality].to(self.device_)
                            for modality in ["atac", "rna"]
                        }
                    break
            batch = {"normed_subgraph": normed_graph, "mask": masks}
            fres = self.model_.gencoder(batch)  # 传递包含 masks 的 batch
    ...
    def differential(
        ...
        v_mean_key: str = "scMMDI_v",
        v_std_key: str = "scMMDI_v_std",
        key_add: str = "scMMDI",
    ) -> None:
