...
class MixtureMultiModalDecoder(BaseMultiOmicsDecoder):
    def __init__(
        ...
        self.weight_dot = nn.Parameter(torch.tensor(0.5, requires_grad=True))
        self.nbatch = nbatch
        self.d_names = distributions
        self.d_styles = distributions_style
        self.mlps = nn.ModuleDict()
        for k, outci in outcs.items():
            ...
            self.mask_weight = nn.Parameter(torch.tensor(0.3).sigmoid())
    ...
    def forward(self, batch: GMINIBATCH, enc_res: FRES) -> FRES:
        ...
        inputs = torch.cat([z, mask_embed * self.mask_weight], dim=1)  # 共享 inputs
    ...
    def step(self, batch: GMINIBATCH, enc_res: FRES) -> Tuple[FRES, LOSS]:
        ...
        for k, disti in fres["dist"].items():
            mask_k = batch["mask"][k]  # [batch_size, feature_dim]
            target_k = batch["output"][k]  # [batch_size, feature_dim]

            # === 第一组：观测数据重建损失 ===
            nlli_obs = -disti.log_prob(batch["output"][k])[batch["mask"][k] == 0.0]

            if nlli_obs.size(0) > 0:
                lossi_obs = self.reductor(nlli_obs, dim=1).mean()
                losses[f"{k}_obs"] = lossi_obs


            # === 第二组：人为掩盖值的重建损失（mask == 1） ===
            nlli_unobs = -disti.log_prob(batch["output"][k])[batch["mask"][k] == 1.0]

            if nlli_unobs.size(0) > 0:
                lossi_unobs = self.reductor(nlli_unobs, dim=1).mean()
                mask_ratio = (batch["mask"][k] == 1.0).float().mean()  # 掩码比例

                losses[f"{k}_unobs"] = lossi_unobs * (0.1 + 0.1 * mask_ratio)  # 动态权重
                #losses[f"{k}_unobs"] = lossi_unobs * 0.3
