import logging
import warnings
from typing import Set, List, Optional, Union, Dict, Tuple, TypedDict, Literal, Sequence, Mapping
...
class ModalMaskGenerator:
    def __init__(self, modality_names: list = None, p_modal: Optional[torch.Tensor] = None, p_random: float = 0.2, max_p_random: float = 0.2):
        """
        初始化随机掩码生成器。

        参数:
        - modality_names: 模态名称列表（如 ["rna", "protein", "atac"]）。
        - p_modal: 每个模态的缺失概率，形状为 (n_modalities,)，默认均匀分布。
        - p_random: 随机掩码的总体概率（控制是否应用随机掩码）。
        """
        self.modality_names = modality_names
        self.n_modalities = len(modality_names)
        if p_modal is None:
            p_modal = torch.ones(self.n_modalities) / self.n_modalities
        self.p_modal = torch.tensor(p_modal, dtype=torch.float32)
        self.p_random = p_random
        self.max_p_random = max_p_random

    def __call__(self, batch: Dict, device: torch.device) -> Dict[str, torch.Tensor]:
        """
        为 batch 生成随机掩码。

        参数:
        - batch: 输入 batch，包含 "mask" 字典，键为模态名称，值为标量掩码（0.0 或 -1.0）。
        - device: 张量设备（如 "cuda:0"）。

        返回:
        - mask: 随机掩码字典，形状与 batch["mask"] 一致，值为 0.0（可观测）、1.0（人为掩盖）或 -1.0（原始缺失）。
        """
        batch_size = next(iter(batch["mask"].values())).shape[0]
        mask = {k: v.clone().to(device) for k, v in batch["mask"].items()}  # 复制原始掩码

        # 决定是否应用随机掩码（以 p_random 概率）
        apply_random = Bernoulli(probs=self.p_random).sample((batch_size,)).to(device)

        data_sizes = {k: (batch["input"][k] != 0).sum().item() for k in self.modality_names}
        total_size = sum(data_sizes.values())
        p_modal_raw = [data_sizes[k] / total_size for k in self.modality_names]
        #p_modal_adjusted = [min(p * 0.4, 0.2) for p in p_modal_raw]  # 限制最大概率为 0.3，乘以 0.6 降低总体掩码率
        #p_modal_adjusted = [p / sum(p_modal_adjusted) for p in p_modal_adjusted]  # 归一化
        self.p_modal = torch.tensor(p_modal_raw, dtype=torch.float32, device=device)
        print(f"p_modal -> {p_modal_raw}")

        for i in range(batch_size):
            if apply_random[i].item() == 0:
                continue  # 不应用随机掩码，保留原始掩码
            # 随机选择一个模态进行掩盖
            modal_idx = Categorical(probs=self.p_modal).sample().item()
            modality = self.modality_names[modal_idx]
            # 确保至少有一个模态可观测
            other_modalities = [m for m in self.modality_names if m != modality]
            has_other_data = any(mask[m][i] == 0.0 for m in other_modalities)
            if has_other_data:
                # 将选中的模态掩码置为 1.0（人为掩盖）
                mask[modality][i] = torch.tensor(1.0, dtype=torch.float32, device=device)
...
class MosaicMuDataset(D.Dataset):
    def __init__(
        ...
        shared_feature_keys: Optional[Mapping[str, Sequence[int]]] = None,  # 新增
        shared_nfeatures: Optional[int] = None,  # 新增
        shared_indices: Optional[Mapping[str, Sequence[int]]] = None,  # 新增
    ) -> None:
        ...
        self._shared_feature_keys = shared_feature_keys
        self._shared_nfeatures = shared_nfeatures
        self._shared_indices = shared_indices
        ...
        # 共享特征维度
        self._shared_feature_ndims = {}
        for key in self._shared_feature_keys:
            for k in self._mdata.mod:
                if key in self._mdata.mod[k].obsm:
                    self._shared_feature_ndims[key] = self._mdata.mod[k].obsm[key].shape[1]
                    break
    ...
    def __getitem__(self, index: int) -> SAMPLE:
        ...
        inpts, outputs, mask, shared_features = {}, {}, {}, {}
        start = 0

        for k, row_ann in row.mod.items():

            # 生成组学级掩码：可观测为 0，缺失为 -1
            data_exists = bool(self._mdata.obsm.get(k, [False])[index])
            mask[k] = torch.tensor(0.0 if data_exists else -1.0, dtype=torch.float32)  # 标量掩码
            #print(f"Modality {k} mask at index {index}: {masks[k].item()}")  # 调试日志
            ...
            if row_ann.n_obs == 0:
                ...
                shared_features[k] = {
                    key: torch.zeros(self._shared_feature_ndims[key], dtype=torch.float32)
                    for key in self._shared_feature_keys if key in self._mdata.mod[k].obsm
                }
            else:
                ...
                shared_features[k] = {
                    key: torch.tensor(
                        to_dense(row_ann.obsm[key]), dtype=torch.float32
                    )
                    for key in self._shared_feature_keys if key in self._mdata.mod[k].obsm
                }
        #print(f"masks={masks}")
        patch = {
            ...
            "output": outputs,
            "mask": mask, # 每模态一个小掩码
            "shared_features": shared_features,  # 新增
...
def get_dataloader(
    ...
    repeat_sample: bool = True,
    drop_last: bool = False,
    shared_feature_keys: Optional[List[str]] = None,  # 新增
    shared_nfeatures: Optional[int] = None,  # 新增
    shared_indices: Optional[Mapping[str, Sequence[int]]] = None,  # 新增
    ...
    mdataset = MosaicMuDataset(
        ...
        shared_feature_keys=shared_feature_keys,  # 新增
        shared_nfeatures=shared_nfeatures,
